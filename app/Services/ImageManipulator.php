...
class ImageManipulator
{
    ...
    public function cropResize($maxW, $maxH)
    {
		if (!is_resource($this->image)) {
            throw new RuntimeException('No image set');
        }
        
		$uploadWidth = $this->getWidth();
		$uploadHeight = $this->getHeight();
        ...
        $temp = imagecreatetruecolor($maxW, $maxH);
        imagecopyresampled($temp, $this->image,  0, 0, $newX, $newY, $maxW, $maxH, $uploadWidth, $uploadHeight);
    ...
    public function save($fileName, $type = IMAGETYPE_JPEG)
    {
        ...
        try {
            switch ($type) {
                case IMAGETYPE_GIF  :
                    if (!imagegif($this->image, $fileName, 85)) {
                        throw new RuntimeException;
                    }
                    break;
                case IMAGETYPE_PNG  :
                    if (!imagepng($this->image, $fileName, 8)) {
                        throw new RuntimeException;
                    }
                    break;
                case IMAGETYPE_JPEG :
                default             :
                    if (!imagejpeg($this->image, $fileName, 85)) {
                        throw new RuntimeException;
                    }
            }
        } catch (Exception $ex) {
